...
{
    u8g2.begin();           //初始化
    u8g2.enableUTF8Print(); //使用UTF-8编码
...
//x 左上角的x坐标
//y 左上角的y坐标
//encoding 字符的unicode值 encoding values, see: https://github.com/olikraus/u8g2/wiki/fntgrpiconic
// 关联方法 setFont
...
void Draw_Weather(int symbol, int degree)
{
    //根据天气代码确定要显示的图标
    if (symbol <= 3)
    {
...
void Draw_Tomorrow_Wheather(int symbol, int degree)
{
...
void Draw_Lunar(uint16_t year, uint8_t month, uint8_t day, unsigned char solar, uint8_t week)
{
    u8g2.setFont(u8g2_font_wqy12_t_gb2312a);
    u8g2.setCursor(0, 12);
