...
void parseInfo(WiFiClient clients)
{
    ...
    Serial.println(F("========================"));
    Draw_Weather(results_0_now_code_int, results_0_now_temperature_int);
}
...
void httpRequest(String reqRes)
{
    ...
    if (clients.connect(host, httpPort))
    {
        ...
        // 向服务器发送http请求信息
        clients.print(httpRequest);
        Serial.println("Sending request: ");
        Serial.println(httpRequest);

        // 获取并显示服务器响应状态行
        String status_response = clients.readStringUntil('\n');
