...
// DS1302 VCC --> 3.3v - 5v
// DS1302 GND --> GND
ThreeWire myWire(/*IO*/ D3, /*SCLK*/ D4, /*RST*/ D5); // IO, SCLK, CE
...
{
    //获取网络时间
    set_year = Blinker.year();
    set_month = Blinker.month();
    set_day = Blinker.mday();
    set_hour = Blinker.hour();
    set_minute = Blinker.minute();
    set_second = Blinker.second();
    set_week = Blinker.wday();
    Rtc.Begin(); //启动RTC模块
    //将全局变量写入RTC芯片
    ...
    clock_minute = clock_time.Minute();
    clock_second = clock_time.Second();

    //异常判断
    if (Rtc.GetIsWriteProtected()) //判断是否有写入保护
    {
        BLINKER_LOG("RTC was write protected, enabling writing now");
        Rtc.SetIsWriteProtected(false); //取消保护
    }

    if (!Rtc.GetIsRunning()) //判断是否运行
    {
        BLINKER_LOG("RTC was not actively running, starting now");
...
void Loop_Time()
{
    ...
    BLINKER_LOG("---Date and Time---", date + time);
    Draw_DateTime(now_time.Year(), now_time.Month(), now_time.Day(), now_time.Hour(), now_time.Minute(), now_time.Second()); //OLED显示                                                                                                   //一秒更新一次
}
